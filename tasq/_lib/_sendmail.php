...
function send_mtpl($mtpl_hashkey, $markers_hash = array(),
	...
	global $msg_sendmail_client_noaddress_notsent, $msg_sendmail_error_sending_bylist, $msg_sendmail_error_emptylist, $msg_sendmail_mtpl, $msg_sendmail_mtpl_notfound_notsent;
...
//		echo " send_tpl ";
		...
		$markers_hash["client_sentmsg"] = "$msg_sendmail_client_noaddress_notsent";
		if ($client_to != "") {
...
//			if ($replyto_admcopy != "") sendmail($replyto_admcopy, $subject, $body, $attm, $client_to);
		...
		$ret = "$msg_sendmail_mtpl [$mtpl_hashkey] $msg_sendmail_mtpl_notfound_notsent";
	}
...
function sendmail($to, $subject, $body, $attm = array(), $reply_to = "") {
	...
	global $msg_sendmail_sentto, $msg_sendmail_error_sendingto, $msg_sendmail_sent_byaddress;
...
//		$is_sent = send_html_mime_mail($mail_from, $to, $subject, $body, $attm, $reply_to);
	...
	if ($debug_sendmail == 1) {
		...
		if ($is_sent) $errormsg .= "$msg_sendmail_sentto [$to_htmlized]";
		else $errormsg .= "$msg_sendmail_error_sendingto $to_htmlized";
	}

	if ($mail_visor != "" && $mail_visor_debug == 1) {
		if ($mail_sendSMTP == 1) {
			$body_visor = $body . "<br><br> $msg_sendmail_sent_byaddress " . $to_htmlized;
